...
public interface SentImageDbRepo extends JpaRepository<SentImageDbEntity, Long> {
    ...
    @Query(value = "SELECT *\n" +
            "FROM sent_images\n" +
            "WHERE conversation_id = :conversation_id AND TIMESTAMP(CONCAT(date, ' ',hour)) < :date\n" +
            "ORDER BY date DESC, hour DESC\n" +
            "LIMIT 100;", nativeQuery = true)
    List<SentImageDbEntity> getRecentImagesAfterDateById(@Param("conversation_id") Long id,
                                                         @Param("date") String date);
    ...
    SentImageDbEntity getMostRecentMessageById(@Param("id") Long id);
}
